...
type Props = { modo: "novo" | "editar"; turmaId?: string; igrejaId: string };
type Professor = {
  id: string;
  nome: string;
  cargo?: string | null;
...
export default function FormEscolaDominical({
  ...
  const carregarListas = useCallback(async () => {
    ...
    try {
      ...
      setErro(getErrorMessage(error, "Erro ao buscar dados do formulário."));
    } finally {
  ...
  const carregarTurma = useCallback(async () => {
    ...
    try {
      ...
      setErro("");
      const [turmaData, alunosTurmaData] = await Promise.all([
        getJsonOrThrow<{
          nome?: string;
          departamento?: string | null;
          professorId?: string;
        }>(`/api/secretaria/escola-dominical/turmas/${turmaId}`),
        getJsonOrThrow<{ alunoIds?: string[] }>(
      ...
      setAlunoIdsSelecionados(
        Array.isArray(alunosTurmaData.alunoIds) ? alunosTurmaData.alunoIds : [],
      );
    } catch (error) {
      setErro(getErrorMessage(error, "Erro ao buscar dados da turma."));
    } finally {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const turmaData = await getJsonOrThrow<{ id?: string }>(endpoint, {
        method,
      ...
      await getJsonOrThrow(
        `/api/secretaria/escola-dominical/turmas/${turmaSalvaId}/alunos`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ igrejaId, alunoIds: alunoIdsSelecionados }),
        },
      );
      setSucesso(
        modo === "novo"
      ...
      setErro(getErrorMessage(error, "Erro ao salvar turma."));
    } finally {
      setSalvando(false);
    }
  }
  ...
  if (loadingPerms) {
    return (
      <div className={styles.container}>
        {" "}
        <div className={styles.stateBox}>Carregando permissões...</div>{" "}
      </div>
    );
  }
    ...
    return (
      <div className={styles.container}>
        ...
        <div className={styles.stateBox}>
          {" "}
          ⛔ Você não tem permissão para {modo === "novo"
            ? "criar"
            : "editar"}{" "}
      ...
      <section className={styles.header}>
        ...
        <div className={styles.headerContent}>
          ...
          <span className={styles.headerTag}>
            {" "}
            {modo === "novo" ? "Novo cadastro" : "Edição de turma"}{" "}
          </span>{" "}
          <h1>
            {" "}
        ...
        <div className={styles.alunosBox}>
          ...
          <div className={styles.alunosHeader}>
            ...
            <div>
              ...
              <p>
                Selecione os membros que participarão da Escola Dominical.
              </p>{" "}
            </div>{" "}
